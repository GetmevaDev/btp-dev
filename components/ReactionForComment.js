...
const ReactionForComment = ({comment}) => {
    ...
    useEffect(() => {
        axios.get(`${process.env.BACKEND_URL}/comment-reactions?_limit=10000&comment.id=${comment.id}`)
            .then(({data}) => {
                console.log(data)
                setReactions(data)
            })
    ...
    const updateReaction = (reaction) => {
        ...
        if (!reactionSelected) {
            ...
            axios.post(`${process.env.BACKEND_URL}/comment-reactions`, {
                type: reaction,
                user: appState.user,
                comment,
            }, config)
            ...
            axios.put(`${process.env.BACKEND_URL}/comment-reactions/${reactionSel.id}`,{
                type: reaction
            }, config)
                .then(({data}) => {
                })
